///
/// hardware model for amdgpu_aldebaran
...
#include "HWModel.h"
#include <stdint.h>
class HWModel_amdgpu_aldebaran : public HWModel {
public:
  HWModel_amdgpu_aldebaran()
      : HWModel("amdgpu_aldebaran", 0x1002, 0x7408, 0, 0, 0x30000),
        probe_len(0) {
  ...
  virtual ~HWModel_amdgpu_aldebaran() {};
  ...
  virtual int read(uint8_t *dest, uint64_t addr, size_t size) final {
    LOG_TO_FILE("qemu_run.log", "HWModel_amdgpu_aldebaran::read dest: 0x"
                                    << std::hex << (uint64_t)dest << std::dec
                                    << " addr: 0x" << std::hex << addr
                                    << std::dec << " size: " << size);
    INFO("HWModel_amdgpu_aldebaran::read dest: 0x" << std::hex <<
    (uint64_t)dest << std::dec << " addr: 0x" << std::hex << addr << std::dec
    << " size: " << size); if (probe_len > 15)
    //   return 0;
