...
///
#include "device-simulator/aplib.h"
#include "shm.h"
...
again:
  ...
  if (!sm->ready) {
    INFO("not ready");
    shm->close();
    INFO("finish closing shm");
    delete shm;
    INFO("finish deleting shm");
    goto again;
  };
  INFO("is ready");
  ...
  LOG_TO_FILE("qemu_run.log", "[Exit] real_ap_init");
};
...
void watch_dog() {
  LOG_TO_FILE("qemu_run.log", "[Enter] watch_dog");
  sleep(130);
...
int ap_get_fuzz_data(uint8_t *dest, uint64_t addr, size_t size, int bar) {
  ...
  return size;
  LOG_TO_FILE("qemu_run.log", "[Exit] ap_get_fuzz_data");
}

void ap_set_fuzz_data(uint64_t data, uint64_t addr, size_t size, int bar) {
  LOG_TO_FILE("qemu_run.log", "[Enter] ap_set_fuzz_data");
  if (IS_DUMP_W) {
    if (dump_rw_addr_to_file) {
  ...
  LOG_TO_FILE("qemu_run.log", "[Exit] ap_set_fuzz_data");
}

void ap_log_pc(uint64_t rip) {
  LOG_TO_FILE("qemu_run.log", "[Enter] ap_log_pc");
  if (!sm)
    ap_init();
  if (!sm)
    return;
...
again:
  ...
  LOG_TO_FILE("qemu_run.log", "[Exit] ap_log_pc");
}

void ap_exit(void) {
  LOG_TO_FILE("qemu_run.log", "[Enter] ap_exit");
  if (shm) {
    // close
    close(fuzz_file_fd);
    fuzz_file_fd = -1;
...
// close:
  ...
  sm = nullptr;
  shm = nullptr;
  LOG_TO_FILE("qemu_run.log", "[Exit] ap_exit");
}
...
int ap_qemu_mmio_read(uint8_t *dest, uint64_t addr, size_t size, int bar) {
  LOG_TO_FILE("qemu_run.log", "[Enter] ap_qemu_mmio_read");
  static int counter;
  static uint64_t cnt;
  uint64_t wrapped_addr;
  auto cur_time = chrono::steady_clock::now();
