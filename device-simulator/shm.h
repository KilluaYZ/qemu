...
private:
  ...
  bool createOpen(size_t s = 0) {
    ...
    LOG_TO_FILE("afl.log", "mmap @fd " << fd << " object size = " << size);
    smem = mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (smem == MAP_FAILED) {
      WARN("cannot mmap shm " << path);
      LOG_TO_FILE("afl.log", "cannot mmap shm " << path);
      smem = nullptr;
      close();
      return false;
    }
  ...
  bool connectOpen(size_t s = 0) {
    ...
    if (fd < 0) {
      ...
      LOG_TO_FILE("afl.log", "cannot open shm " << shmPath<<" error "<<errno);
      return false;
    }
    size_t size = s;
    if (s == 0) {
